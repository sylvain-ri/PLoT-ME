...
def test_table_rev_comp_to_forward_strand(k, d_codons_orig_target, d_template_counts_combined, ar_codons_forward_addr, ar_codons_rev_comp_addr):
...
seq_w_expected_counts = [
    ...
    {'k':3, 'seq': "AAAACCCCGGGGTTTT",
     'counts': {"AAA":2, "AAC":1, "AAG":0, "AAT":0, "ACA":0, "ACC":0, "ACG":0, "ACT":0, "AGA":0, "AGC":0, "AGG":0, "AGT":0, "ATA":0, "ATC":0, "ATG":0, "ATT":0,
                "CAA":0, "CAC":0, "CAG":0, "CAT":0, "CCA":0, "CCC":2, "CCG":1, "CCT":0, "CGA":0, "CGC":0, "CGG":0, "CGT":0, "CTA":0, "CTC":0, "CTG":0, "CTT":0,
                "GAA":0, "GAC":0, "GAG":0, "GAT":0, "GCA":0, "GCC":0, "GCG":0, "GCT":0, "GGA":0, "GGC":0, "GGG":2, "GGT":1, "GTA":0, "GTC":0, "GTG":0, "GTT":0,
...
def test_bio_seq_count_kmer(k, seq, counts, np_counts):
    assert bio.seq_count_kmer(seq, k) == counts

@pytest.mark.parametrize("k, seq, counts, np_counts", seq_w_expected_counts_tuples)
def test_cyt_seq_count_kmer(k, seq, counts, np_counts):
    assert cyt_ext.kmer_counter(seq, k, dictionary=True)  == counts
    assert cyt_ext.kmer_counter(seq, k, dictionary=False) == np_counts
